...
describe("Startup configuration", function() {
    ...
    describe("Correlation IDs", function() {
        let correlationId = require("correlation-id");

        it("can be added to the logs", function() {
            let layout = layouts.layout("json");
            let event = new LoggingEvent("foo", log4js.levels.INFO, ["message"]);

            let result = correlationId.withId("abc123", () => JSON.parse(layout(event)));

            expect(result).to.have.property("correlation-id", "abc123");
        });

        it("are added to requests", function() {
            let http = require("http");

            let request = correlationId.withId("header321", () => {
                return http.get("http://localhost:3000", {
                    headers: { foo: "bar" }, lookup: () => {}
                });
            });

            let header = request.getHeader("x-correlation-id");
            expect(header).to.equal("header321");
        });
    });
});